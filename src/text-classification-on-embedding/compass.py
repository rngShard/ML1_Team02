...
def plot_user(party_name,politician_name,probabilities_list ):
    __parties_x = [4,7,5,2,-3,3,8,-5]
    __parties_y = [7,5,3,4,-2,2,5.5,-5]
    __parties_names = ['CSU', 'FDP', 'AFD', 'Die Grunen', 'Die Linke', 'SPD', 'CDU','' ]
    __parties_colors = ['c','y', 'b', 'g', 'm', 'r','k','w']
    ...
    ax.scatter(__parties_x, __parties_y, c =__parties_colors)
    for i, __parties_names in enumerate(__parties_names):
        ax.annotate(__parties_names, (__parties_x[i],__parties_y[i]))

        
    politician_x = 0
    politician_y = 0
    for j in range(0,len(probabilities_list)):
        politician_x += probabilities_list[j] * __parties_x[j]
        politician_y += probabilities_list[j] * __parties_y[j]
    ...
    plt.savefig('plots/'+party_name+'/'+politician_name+'')
...
class PoliticalCompass:
    ...
    def plotPoliticianInCompass(self, party_name,politician_name_list, probabilities_list):
        ...
        for i in range(0, len(politician_name_list)):
            ...
            plt.gcf().canvas.set_window_title('Political Compass')
            plt.savefig(party_name+'/'+politician_name_list[i]+'')
